...
object BenchUtils {
  ...
  def runBench(
    ...
    for (i <- 0 until iterations) {
      ...
      df = createDataFrame(spark).withColumn("timestamp", current_timestamp()).withWatermark("timestamp","1 second")

      resultsAction match {
        case Collect() => df.collect()
        case WriteCsv(path, mode, options) =>
          //df.write.mode(mode).options(options).csv(path)
          df.writeStream.outputMode("complete").format("console").option("path", path)
          .option("checkpointLocation", "./ck").start().awaitTermination()
